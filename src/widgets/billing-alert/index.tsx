...
function AlertCard({ alert, showCta }: { alert: BillingAlert; showCta: boolean }) {
  const isNegative = alert.severity === "negative";
  const accent = isNegative ? "var(--w-status-negative)" : "var(--w-status-cautionary)";
  ...
  return (
    <Card className="flex items-center gap-4" style={{ borderColor: accent, background: tint }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ font: "600 14.5px/1.3 var(--w-font-sans)", color: "var(--w-fg-strong)" }}>
          {alert.message}
        </div>
        <div style={{ font: "500 12.5px/1.5 var(--w-font-sans)", color: "var(--w-fg-neutral)", marginTop: 3 }}>
          청구 및 결제 페이지에서 확인하고 Meta 결제 페이지로 이동해 처리할 수 있어요.
        </div>
      </div>
      {showCta && (
